...
```{r, fig.cap = "Mixed-effect effect size estimates for all conditions (red) and each of the four theoretically-relevant conditions in our sample. Ranges are 95% confidence intervals. Point size corresponds to sample size. FN = Familiar-Novel trials; NN = Novel-Novel trials." , fig.height = 3, fig.width = 9, fig.pos = "t"}
...
colnames(print_age_table_data) <- c("Model", "n", "term", "estimate", "Z", "p")
```

```{r, results = "asis"}
apa_table(print_age_table_data, caption = "Meta-analytic model parameters for model including age as a fixed effect. The first model (top) estimates effect sizes for all studies in our sample. The four subsequent models present separate models paremeters for four seperate conditions. Ranges are 95\\% confidence intervals.", small = T, note = "n = sample size (number of studies); FN = Familiar-Novel; NN = Novel-Novel.")
```
...
at_a_pval <- ifelse(round(TFN_NNN_model$pval[1],2)  < .0001,
                    "<.0001", paste0(" = ", round(TFN_NNN_model$pval[1],2)))
at_t_pval <- ifelse(round(TFN_NNN_model$pval[2],2)  < .0001, 
...
- new paper from Hutchinson, Ron Pomper paper from merriman
- check sub sample
- check alvaros ones

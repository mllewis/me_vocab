...
intercept <- mod_print %>%
  ...
  kable(print_age_table_data, 
        ...
        format = "latex", 
        booktabs = TRUE,
        caption = "Meta-analytic model parameters for 5 different models. Each model predicts effect size with age as a fixed effect.")  %>%
...
av_a_pval <- round(age_vocab_model$pval[1],2)
av_v_pval <- round(age_vocab_model$pval[2],2)
```
...
at_a_pval <- round(TFN_NNN_model$pval[1],2)
at_t_pval <- round(TFN_NNN_model$pval[2],2)
```
...
ml_a_pval <- round(mono_vs_multi_model$pval[2],2)
ml_l_pval <- round(mono_vs_multi_model$pval[3],2)
```
...
tnta_a_pval <- round(T_vs_NT_model$pval[2],2)
tnta_tnt_pval <- round(T_vs_NT_model$pval[3],2)
```

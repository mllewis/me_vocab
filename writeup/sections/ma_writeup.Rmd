...
To estimate the overall effect size, we fit a mixed effect model for the full sample of conditions. Effect size estimates are presented in Figure 1. The overall effect size estimate reliably differed from zero (*d* = `r filter(all_overall_estimates, model_name == "grand") %>% select(es_string) %>% as.character()`; see Appendix for by-condition forest plots). We also fit additional models for sub-populations in our sample (d~typically-developing~ = `r filter(all_overall_estimates, model_name == "typical") %>% select(es_string) %>% as.character()`; *d*~multilingual~ = `r filter(all_overall_estimates, model_name == "typical") %>% select(es_string) %>% as.character()`; *d*~non-typically-developing~ = `r filter(all_overall_estimates, model_name == "NT") %>% select(es_string) %>% as.character()`). In the next set of analyses, we ask whether our moderator variables predict variance in these overall effect size estimates. 
